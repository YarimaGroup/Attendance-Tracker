...
class SummaryPanel extends StatelessWidget {
  ...
  final DateTime from;
  final DateTime to;
  const SummaryPanel({super.key, required this.rows, required this.groupByUser, required this.from, required this.to});
  ...
  Widget build(BuildContext context) {
    ...
    final totalWorkingTime = groupByUser
        ? buildGroups(rows).fold<Duration>(Duration.zero, (acc, g) => acc + g.workingTime)
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(children: [
            Expanded(child: SummaryStat(icon: Icons.receipt_long, label: 'Total Records', value: '$totalRecords', color: Colors.blue)),
            const SizedBox(width: 8),
            Expanded(child: SummaryStat(icon: Icons.login, label: 'Clock Ins', value: '$clockIns', color: Colors.green)),
            const SizedBox(width: 8),
            Expanded(child: SummaryStat(icon: Icons.logout, label: 'Clock Outs', value: '$clockOuts', color: Colors.orange)),
            const SizedBox(width: 8),
            Expanded(child: SummaryStat(icon: Icons.people, label: 'Unique Users', value: '$uniqueUsers', color: Colors.purple)),
          ]),
...
class SummaryStat extends StatelessWidget {
  ...
  final String value;
  final Color color;
  const SummaryStat({super.key, required this.icon, required this.label, required this.value, required this.color});
...
class WorkingTimeCard extends StatelessWidget {
  ...
  const WorkingTimeCard({super.key, required this.totalWorkingTime, required this.uniqueUsers});
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [Colors.indigo.withOpacity(0.1), Colors.blue.withOpacity(0.05)],
          ...
          if (uniqueUsers > 0)
            Column(
              ...
              children: [
                ...
                Text(
                  formatDuration(Duration(milliseconds: totalWorkingTime.inMilliseconds ~/ uniqueUsers)),
